...
This document defines the agent-friendly exception pattern for `jankurai-deploy`:
...
2. Add an entry to the relevant `agent/*.toml` manifest. For boundary
   reclassifications use the `[[reclassification]]` block in
   [`agent/boundaries.toml`](../agent/boundaries.toml); for scan scope use the
   `[scan]` section of [`agent/audit-policy.toml`](../agent/audit-policy.toml).
...
A temporary boundary reclassification is recorded in
...
owner = "ops"
expires = "2026-12-31"
migration_path = "Fold the legacy deploy scripts into ops/ci lanes and drop this entry."
```
...
Secret leakage, destructive migrations without rollback, unsigned release
artifacts, and hand-edits to generated zones are never granted exceptions. Fix
